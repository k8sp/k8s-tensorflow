...
3. 创建、编辑yaml：
   a. 创建一个ps service(对应一个ps pod)；
   b. 创建两个worker service(分别对应一个worker pod)
...
4. 获取tensorflow训练代码和数据
   大致有两个途径：
   a. 把代码和数据build进images，再分发到k8s节点。
   b. 分发clean的tf image到k8s节点，通过command字段运行一个启动脚本，从外网curl到container，或挂载一个内网的nfs目录(或ceph)到container
